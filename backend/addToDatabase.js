...
function writeBuildingsToDatabase(buildings){
  ...
  db.serialize(function() {
    ...
    //create table in the current database
    sqlString = "CREATE TABLE buildings (name STRING, lat INTEGER, long INTEGER, average_rating REAL);";
    ...
    insert, or inject, SQL commands for the ? parameters.
    */
    var stmt = db.prepare("INSERT INTO buildings (name,lat,long,average_rating) VALUES (?,?,?,?);");
    for (var i = 0; i < buildings.length; i++) {
      building = buildings[i];
      stmt.run(building.name,building.lat,building.long,building.average_rating);
    }
    stmt.finalize();
